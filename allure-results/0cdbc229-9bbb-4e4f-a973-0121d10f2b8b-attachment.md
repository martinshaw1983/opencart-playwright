...
- Name: search-products.spec.ts >> Search for product
- Location: tests/search-products.spec.ts:20:5
    ...
    - generic [ref=e76]:
      ...
      - generic [ref=e112]:
        ...
        - generic [ref=e130]:
          ...
          - textbox "Qty" [active] [ref=e132]: MacBook
...
```ts
  ...
  4  | import { ProductPage } from '../pages/product-page'
  5  | 
  6  | let config: TestConfig;
  7  | let searchResultsPagePage: SearchResultsPage;
  8  | let productPage: ProductPage;
  9  | 
  10 | test.beforeEach(async ({ page }) => {
  11 |     config = new TestConfig(); // Load config (URL, credentials)
  12 | 
  13 |     // Initalise page objects
  14 |     searchResultsPagePage = new SearchResultsPage(page);
  15 |     productPage = new ProductPage(page);
  16 | 
  17 |     await page.goto(config.appUrl); // Navigate to base URL
  18 | });
  19 | 
  20 | test('Search for product', async({page})=>{
  21 |     let product = 'MacBook'
  22 |     await searchResultsPagePage.enterSearchTerm(product);
  23 |     await searchResultsPagePage.clickSearch();
  24 |     await expect(searchResultsPagePage.searchResults.first()).toContainText(product, {ignoreCase: true});
  25 |     await searchResultsPagePage.selectProduct(product);
  26 |     await productPage.addProductToCart(product);
> 27 |     await expect(productPage.msgAddedToCart).toBeVisible();
     |                                              ^ Error: expect(locator).toBeVisible() failed
  28 | });
```