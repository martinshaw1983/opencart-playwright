...
- Location: tests/search-products.spec.ts:17:5

# Error details

```
Error: locator.click: Error: strict mode violation: locator('h4>a').filter({ hasText: 'macbook' }) resolved to 3 elements:
...
Call log:
  - waiting for locator('h4>a').filter({ hasText: 'macbook' })
...
```ts
  ...
  10 |         super(page);
  11 |         this.searchPageHeader = page.locator('#content > h1')
  12 |         this.searchResults = page.locator('h4>a');
  13 |     }
  14 | 
  15 |     async selectProduct(productName: string) {
  16 |         // This finds the specific link that has your product name
  17 |         const product = this.searchResults.filter({ hasText: productName });
  18 |         // Playwright waits for it to be ready and clicks it
> 19 |         await product.click();
     |                       ^ Error: locator.click: Error: strict mode violation: locator('h4>a').filter({ hasText: 'macbook' }) resolved to 3 elements:
  20 |     }
  21 | 
  22 |     async getProductCount(): Promise<number> {
  23 |         return await this.searchResults.count();
  24 |     }
  25 | }
```