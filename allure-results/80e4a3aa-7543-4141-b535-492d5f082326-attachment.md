...
Locator: locator('h4>a')
Expected substring: "macbook"
Error: strict mode violation: locator('h4>a') resolved to 3 elements:
...
```ts
  ...
  4  | 
  5  | let config: TestConfig
  6  | let searchResultsPagePage: SearchResultsPage
  7  | 
  8  | test.beforeEach(async ({ page }) => {
  9  |     config = new TestConfig(); // Load config (URL, credentials)
  10 | 
  11 |     // Initalise page objects
  12 |     searchResultsPagePage = new SearchResultsPage(page);
  13 | 
  14 |     await page.goto(config.appUrl); // Navigate to base URL
  15 | });
  16 | 
  17 | test('Search for product', async({page})=>{
  18 |     let product = 'macbook'
  19 |     await searchResultsPagePage.enterSearchTerm(product);
  20 |     await searchResultsPagePage.clickSearch();
> 21 |     await expect (searchResultsPagePage.searchResults).toContainText(product, {ignoreCase: true});
     |                                                        ^ Error: expect(locator).toContainText(expected) failed
  22 | });
```