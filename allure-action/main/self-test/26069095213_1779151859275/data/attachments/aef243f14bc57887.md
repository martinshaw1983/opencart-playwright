...
- Name: search-products.spec.ts >> Search for product and select product
- Location: tests/search-products.spec.ts:12:5
...
Call log:
  - waiting for locator('h4>a').filter({ has: getByText('MacBook', { exact: true }) })
    - locator resolved to <a href="https://tutorialsninja.com/demo/index.php?route=product/product&product_id=43&search=MacBook">MacBook</a>
...
```ts
  ...
  14 |         this.noProductsFound = page.getByText('There is no product that');
  15 |     }
  16 | 
